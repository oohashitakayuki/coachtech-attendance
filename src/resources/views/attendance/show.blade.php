...
<div class="attendance-detail">
  ...
  @if ($correct && !$correct->approved_at)
  <div class="attendance-detail__inner">
    <div class="attendance-detail__content">
      <div class="attendance-detail__group">
        <label class="attendance-detail__label">名前</label>
        <p class="attendance-detail__user-name">{{ $correct->attendance->user->name }}</p>
      </div>

      <div class="attendance-detail__group">
        <label class="attendance-detail__label">日付</label>
        <p class="attendance-detail__date">{{ \Carbon\Carbon::parse($correct->attendance->date)->format('Y年') }}</p>
        <p class="attendance-detail__date">{{ \Carbon\Carbon::parse($correct->attendance->date)->format('n月j日') }}</p>
      </div>

      <div class="attendance-detail__group">
        <label class="attendance-detail__label">出勤・退勤</label>
        <p class="attendance-detail__work-start">{{ \Carbon\Carbon::parse($correct->attendance->work_start)->format('H:i') }}</p>
        <span class="time-range__tilde">〜</span>
        <p class="attendance-detail__work-end">{{ \Carbon\Carbon::parse($correct->attendance->work_end)->format('H:i') }}</p>
      </div>

      @foreach ($correct->attendance->rests as $index => $rest)
      <div class="attendance-detail__group">
        <label class="attendance-detail__label">{{ $index === 0 ? '休憩' : '休憩' . ($index + 1) }}</label>
        <p class="attendance-detail__break-start">{{ \Carbon\Carbon::parse($rest->break_start)->format('H:i') }}</p>
        <span class="time-range__tilde">〜</span>
        <p class="attendance-detail__break-end">{{ \Carbon\Carbon::parse($rest->break_end)->format('H:i') }}</p>
      </div>
      @endforeach

      <div class="attendance-detail__group">
        <label class="attendance-detail__label">備考</label>
        <p class="attendance-detail__comment">{{ $correct->comment }}</p>
      </div>
    </div>

    <p class="attendance-detail__attention">*承認待ちのため修正はできません。</p>
  </div>

  @else
  <form class="attendance-detail__inner" action="{{ route('attendance.storeCorrectRequest') }}" method="post">
    <input type="hidden" name="date" value="{{ $attendance->date }}">
    @csrf
    <div class="attendance-detail__content">
      <div class="attendance-detail__group">
        <label class="attendance-detail__label">名前</label>
        <p class="attendance-detail__user-name">{{ $user->name }}</p>
      </div>
